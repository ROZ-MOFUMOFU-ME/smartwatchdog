...
6. **定期実行**: Cron Triggerで30分間隔で自動実行（CrowdSec等に弾かれる場合は更に延長可）
...
| 課題 | 対策環境変数 | 推奨初期値例 |
| ---- | ------------- | ------------ |
| サーバー側 429/503 | `CHECK_CONCURRENCY` | 3〜6 |
...
必要に応じてこれらを組み合わせ、Cloudflare Cron の間隔(例: 10→30分)調整も検討してください。
